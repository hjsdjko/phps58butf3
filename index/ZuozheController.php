...
session_start();
class ZuozheController extends CommonController {
    ...
    public function login(){
        ...
        $sql = "select * from `zuozhe` where `zuozhezhanghao` = '".$username."' and `mima` = '".$password."'";
    ...
    public function register(){
        ...
        $colum = "zuozhezhanghao";
        $trues = "select * from `zuozhe` where `zuozhezhanghao` = '".$postData[$colum]."'";
        ...
        if($result->num_rows<1){
			...
			$key = implode(',',$keyArr);
			$v = implode(',',$valArr);
            $sql = "INSERT INTO `zuozhe` (`id`,".$key.") VALUES (".time().",".$v.")";
            $result = table_sql($sql);
    ...
    public function resetPass(){
        ...
        $counts = "select * from `zuozhe` where zuozhezhanghao = '".$username."'";
        $cotte = table_sql($counts);
        if($cotte->num_rows<1){
            exit(json_encode(['code'=>500,'msg'=>"用户名错误。"]));
        }
        $sql = "update zuozhe set mima = '123456' where zuozhezhanghao = '".$username."'";
        $result = table_sql($sql);
    ...
    public function page(){
        ...
        $lists = "select * from `zuozhe` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function list(){
        ...
        $page_count = ceil($numberCount/$limt);//页数
        $startCount = ($page-1)*$limt;
        $lists = "select * from `zuozhe` ".$where." order by ".$sortorders." limit ".$startCount.",".$limt;
        $result = table_sql($lists);
        $arrayData = array();
        if ($result->num_rows > 0) {
            while ($datas = $result->fetch_assoc()){
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
            $zuozhezhanghao = $postData['zuozhezhanghao'];
            $resultInfo = table_sql("select zuozhezhanghao from `zuozhe` where `zuozhezhanghao` = '".$zuozhezhanghao."'");
            if ($resultInfo->num_rows > 0) {
                exit(json_encode(['code'=>500,'msg'=>"作者账号已存在"]));
            }
            foreach ($postData as $key => $value){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
        $sql = "INSERT INTO `zuozhe` (`id`,".$k.") VALUES (".time().",".$v.")";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function add(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
        $sql = "INSERT INTO `zuozhe` (".$k.") VALUES (".$v.")";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function update(){
        ...
        $postData = json_decode($tmpData,true);
            $zuozhezhanghao = $postData['zuozhezhanghao'];
            $id = $postData['id'];
            $resultInfo = table_sql("select zuozhezhanghao from `zuozhe` where id <> ".$id." and `zuozhezhanghao` = '".$zuozhezhanghao."'");
            if ($resultInfo->num_rows > 0) {
                exit(json_encode(['code'=>500,'msg'=>"作者账号已存在"]));
            }
        $v = array();
        ...
        $value = implode(',',$v);
        $sql = "UPDATE zuozhe SET ".$value." where id = ".$id;
        $result = table_sql($sql);
    ...
    public function delete(){
        ...
        $sql = "delete from zuozhe WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        $sql = "select * from `zuozhe` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            while($row = $result->fetch_assoc()) {
    ...
    public function value(){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
        $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM zuozhe ".$where." group by ".$xColumnName;
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM zuozhe ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d')";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM zuozhe ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m')";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM zuozhe ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y')";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function valueMul(){
        ...
        foreach(explode(",", $yColumnName) as $item){
            $sql = "SELECT ".$xColumnName.",sum(".$item.") total FROM zuozhe ".$where." group by ".$xColumnName." LIMIT 10";
            if(urldecode($request[6]) == '日') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$item.") total FROM zuozhe ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') LIMIT 10";
            }
            if(urldecode($request[6]) == '月') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$item.") total FROM zuozhe ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m') LIMIT 10";
            }
            if(urldecode($request[6]) == '年') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$item.") total FROM zuozhe ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y') LIMIT 10";
            }
            $result = table_sql($sql);
    ...
    public function group($columnName){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
        $sql = "SELECT ".$columnName.",count(".$columnName.") as total FROM zuozhe ".$where." GROUP BY ".$columnName." ORDER BY $columnName asc";
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            $total = array();
