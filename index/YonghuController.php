...
class YonghuController extends CommonController {
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
            $zhanghao = $postData['zhanghao'];
            $resultInfo = table_sql("select zhanghao from `yonghu` where `zhanghao` = '".$zhanghao."'");
            if ($resultInfo->num_rows > 0) {
                exit(json_encode(['code'=>500,'msg'=>"账号已存在"]));
            }
            foreach ($postData as $key => $value){
    ...
    public function update(){
        ...
        $postData = json_decode($tmpData,true);
            $zhanghao = $postData['zhanghao'];
            $id = $postData['id'];
            $resultInfo = table_sql("select zhanghao from `yonghu` where id <> ".$id." and `zhanghao` = '".$zhanghao."'");
            if ($resultInfo->num_rows > 0) {
                exit(json_encode(['code'=>500,'msg'=>"账号已存在"]));
            }
        $v = array();
    ...
    public function shBatch(){
        ...
        preg_match_all('/\d+/',$ids,$arr);
        $str = implode(',',$arr[0]);//拼接字符，
        $sql = "update yonghu set sfsh = '".$_GET['sfsh']."', shhf='".$_GET['shhf']."' WHERE id in(".$str.")";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
