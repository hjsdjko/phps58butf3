...
session_start();
class RemenxiaoshuoController extends CommonController {
    ...
    public function page(){
        ...
        if($tokens['tablename'] == 'zuozhe') {
            $where .= " and `zuozhezhanghao` = '".$tokens['username']."' ";
        }
        ...
        if ($base['isAdmin']!=1 || ($base['isAdmin']==1 && $base['tablename'] != "users")){
            ...
            $colum = "zuozhezhanghao";
            $columData = $base['columData'];
            if (isset($_SESSION[$md5]) && in_array($colum, $columData)){
                if($base['tablename'] == 'zuozhe'){
                    $orsign = false;
                    if(!$orsign) {
                        $where .= " and `zuozhezhanghao` = '".$_SESSION[$md5]."'";
                    }
                }
            }
        }
        
        $sql = "select * from `remenxiaoshuo` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $lists = "select * from `remenxiaoshuo` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function list(){
        ...
        $page_count = ceil($numberCount/$limt);//页数
        $startCount = ($page-1)*$limt;
        $lists = "select * from `remenxiaoshuo` ".$where." order by ".$sortorders." limit ".$startCount.",".$limt;
        $result = table_sql($lists);
        $arrayData = array();
        if ($result->num_rows > 0) {
            while ($datas = $result->fetch_assoc()){
    ...
    public function save(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
        $sql = "INSERT INTO `remenxiaoshuo` (".$k.") VALUES (".$v.")";
        $result = table_sql($sql);
    ...
    public function update(){
        ...
        $value = implode(',',$v);
        $sql = "UPDATE remenxiaoshuo SET ".$value." where id = ".$id;
        $result = table_sql($sql);
    ...
    public function delete(){
        ...
        $sql = "delete from remenxiaoshuo WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        $this->andone('remenxiaoshuo',$id,$column='clicknum',$num=1);
        $clicktime = "update remenxiaoshuo set clicktime = '".date('Y-m-d H:i:s',time())."' where ".$where;
        table_sql($clicktime);
        $sql = "select * from `remenxiaoshuo` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function value(){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
        if($tokens['isAdmin'] != 1 && $tokens['tablename'] == 'zuozhe') {
            $where .= " and `zuozhezhanghao` = '".$tokens['username']."' ";
        }
        $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM remenxiaoshuo ".$where." group by ".$xColumnName;
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM remenxiaoshuo ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d')";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM remenxiaoshuo ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m')";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM remenxiaoshuo ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y')";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function valueMul(){
        ...
        foreach(explode(",", $yColumnName) as $item){
            $sql = "SELECT ".$xColumnName.",sum(".$item.") total FROM remenxiaoshuo ".$where." group by ".$xColumnName." LIMIT 10";
            if(urldecode($request[6]) == '日') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$item.") total FROM remenxiaoshuo ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') LIMIT 10";
            }
            if(urldecode($request[6]) == '月') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$item.") total FROM remenxiaoshuo ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m') LIMIT 10";
            }
            if(urldecode($request[6]) == '年') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$item.") total FROM remenxiaoshuo ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y') LIMIT 10";
            }
            $result = table_sql($sql);
    ...
    public function thumbsup($id=false){
        ...
        if ($type==1){
            $result = $this->andone('remenxiaoshuo',$id,$column = 'thumbsupnum',$num = 1);
        }else{
            $result = $this->andone('remenxiaoshuo',$id,$column = 'crazilynum',$num = 1);
        }
    ...
    public function autoSort(){
        ...
        if ($where) {
		    $lists = "select * from `remenxiaoshuo` where ".$where." order by ".$sort." desc limit 0,".$limt;
        }else{
		    $lists = "select * from `remenxiaoshuo` order by ".$sort." desc limit 0,".$limt;
        }
		$result = table_sql($lists);
		$arrayData = array();
		if ($result->num_rows > 0) {
    ...
    public function group($columnName){
        ...
        $sql = "SELECT ".$columnName.",count(".$columnName.") as total FROM remenxiaoshuo ".$where." GROUP BY ".$columnName." ORDER BY $columnName asc";
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            $total = array();
    ...
    public function shBatch(){
        ...
        preg_match_all('/\d+/',$ids,$arr);
        $str = implode(',',$arr[0]);//拼接字符，
        $sql = "update remenxiaoshuo set sfsh = '".$_GET['sfsh']."', shhf='".$_GET['shhf']."' WHERE id in(".$str.")";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
