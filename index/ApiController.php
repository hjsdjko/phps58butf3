...
class ApiController extends CommonController
{
    ...
    public function download()
    {
        ...
        header('location:http://' . $_SERVER['HTTP_HOST'] . '/phps58butf3/upload/' . $fileName);
        exit; 
    }
    /**
     * 获取某表的某个字段列表接口
    ...
    public function value($tableName,$xColumnName,$yColumnName,$tjType){
        ...
        $where = " where 1 ";
        if ($tokens['isAdmin']!=1 || ($tokens['isAdmin']==1 && $tokens['tablename'] != "users")){
            if($tableName == 'remenxiaoshuo'){
                if($tokens['tablename'] == 'zuozhe'){
                    $where .= " and `zuozhezhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tableName == 'wodeshujia'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `zhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tableName == 'xiaoshuotingshu'){
                if($tokens['tablename'] == 'yonghu'){
                    $where .= " and `zhanghao` = '".$tokens['username']."' ";
                }
            }
