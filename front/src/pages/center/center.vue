...
<div class="center-preview" :style='{"padding":"0 7%","margin":"20px auto 40px","flexWrap":"wrap","background":"none","display":"block","width":"100%","position":"relative","justifyContent":"space-between"}'>
	...
	<div class="info" :style='{"padding":"50px 20px 10px","boxShadow":"0 0px 0px rgba(0, 0, 0, 0.3)","margin":"20px 0 0 0","borderColor":"#28890b20","alignItems":"center","display":"flex","right":"7%","borderRadius":"0px","flexWrap":"wrap","background":"#fff","borderWidth":"0px","width":"calc(86% - 260px)","position":"absolute","borderStyle":"solid","height":"auto","order":"2"}'>
		...
		<div :style='{"width":"auto","borderColor":"#1abc9e20","fontSize":0,"borderStyle":"solid","borderWidth":"0 0 0px 0","height":"auto"}' v-if="userTableName=='zuozhe'">
			<el-image :style='{"margin":"10px auto","borderColor":"#efefef","objectFit":"cover","borderRadius":"100%","borderWidth":"0 0 1px 0","display":"block","width":"90px","borderStyle":"solid","height":"90px"}' :src="sessionForm.touxiang?baseUrl + sessionForm.touxiang:require('@/assets/avator.png')" fit="cover"></el-image>
		</div>
		...
		<div :style='{"padding":"0 20px","borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 0px 0","display":"flex","width":"auto","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='yonghu'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"padding":"0 5px","fontSize":"14px","color":"#333","display":"none"}'></span>
			<div :style='{"color":"#666","margin":"0 10px 0 0","fontSize":"14px"}'>是否审核</div>
			<div :style='{"fontSize":"14px","color":"#999","textAlign":"left","flex":1}'>{{sessionForm.sfsh}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 0px 0","display":"flex","width":"auto","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='zuozhe'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"padding":"0 5px","fontSize":"14px","color":"#333","display":"none"}'></span>
			<div :style='{"color":"#666","margin":"0 10px 0 0","fontSize":"14px"}'>作者账号</div>
			<div :style='{"fontSize":"14px","color":"#999","textAlign":"left","flex":1}'>{{sessionForm.zuozhezhanghao}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 0px 0","display":"flex","width":"auto","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='zuozhe'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"padding":"0 5px","fontSize":"14px","color":"#333","display":"none"}'></span>
			<div :style='{"color":"#666","margin":"0 10px 0 0","fontSize":"14px"}'>作者姓名</div>
			<div :style='{"fontSize":"14px","color":"#999","textAlign":"left","flex":1}'>{{sessionForm.zuozhexingming}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 0px 0","display":"flex","width":"auto","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='zuozhe'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"padding":"0 5px","fontSize":"14px","color":"#333","display":"none"}'></span>
			<div :style='{"color":"#666","margin":"0 10px 0 0","fontSize":"14px"}'>手机号</div>
    ...
    <el-tabs tab-position="left" :style='{"width":"100%","margin":"20px 0 0","background":"none"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="100px">
          <el-form-item :style='{"width":"auto","padding":"0px","margin":"0 0 40px","background":"none","display":"inline-block"}' v-if="userTableName=='yonghu'" label="账号" prop="zhanghao">
          ...
          <el-form-item class="balance" :style='{"width":"auto","padding":"0px","margin":"0 0 40px","background":"none","display":"inline-block"}' v-if="userTableName=='yonghu'" label="会员">
            <div :style='{"flexWrap":"wrap","display":"flex"}'>
				<el-input v-model="sessionForm.vip" placeholder="会员" readonly></el-input>
				<div @click="dialogFormVisibleVip = true" :style='{"cursor":"pointer","padding":"0 15px","margin":"0 20px 0 10px","borderColor":"#eee","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#1e3c4f","borderWidth":"0px","width":"auto","lineHeight":"36px","fontSize":"14px","borderStyle":"solid","height":"36px"}'>会员购买</div>
			</div>
		  </el-form-item>
          <el-form-item :style='{"width":"auto","padding":"0px","margin":"0 0 40px","background":"none","display":"inline-block"}' v-if="userTableName=='zuozhe'" label="作者账号" prop="zuozhezhanghao">
            <el-input v-model="sessionForm.zuozhezhanghao" placeholder="作者账号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"auto","padding":"0px","margin":"0 0 40px","background":"none","display":"inline-block"}' v-if="userTableName=='zuozhe'" label="作者姓名" prop="zuozhexingming">
            <el-input v-model="sessionForm.zuozhexingming" placeholder="作者姓名" ></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"auto","padding":"0px","margin":"0 0 40px","background":"none","display":"inline-block"}' v-if="userTableName=='zuozhe'" label="手机号" prop="shoujihao">
            <el-input v-model="sessionForm.shoujihao" placeholder="手机号" ></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"auto","padding":"0px","margin":"0 0 40px","background":"none","display":"inline-block"}' v-if="userTableName=='zuozhe'" label="性别" prop="xingbie">
            <el-select v-model="sessionForm.xingbie" placeholder="请选择性别" >
              <el-option v-for="(item, index) in dynamicProp.xingbie" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item :style='{"width":"auto","padding":"0px","margin":"0 0 40px","background":"none","display":"inline-block"}' v-if="userTableName=='zuozhe'" label="头像" prop="touxiang">
			<file-upload
			tip="点击上传头像"
			action="file/upload"
			:limit="1"
			:multiple="true"
			:fileUrls="sessionForm.touxiang?sessionForm.touxiang:''"
			@change="zuozhetouxiangHandleAvatarSuccess"
			></file-upload>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
		...
		if ('zuozhe' == this.userTableName) {
			this.$set(this.sessionForm, 'zuozhezhanghao', null);
		}
		if ('zuozhe' == this.userTableName) {
			this.$set(this.sessionForm, 'zuozhexingming', null);
		}
		if ('zuozhe' == this.userTableName) {
			this.$set(this.sessionForm, 'mima', null);
		}
		...
		if ('zuozhe' == this.userTableName&&this.rules['zuozhezhanghao']){
			this.rules['zuozhezhanghao'].push({ required: true, message: '请输入作者账号', trigger: 'blur' })
		}else if('zuozhe' == this.userTableName&&!this.rules['zuozhezhanghao']) {
			this.$set(this.rules, 'zuozhezhanghao', [{ required: true, message: '请输入作者账号', trigger: 'blur' }]);
		}
		if ('zuozhe' == this.userTableName&&this.rules['zuozhexingming']){
			this.rules['zuozhexingming'].push({ required: true, message: '请输入作者姓名', trigger: 'blur' })
		}else if('zuozhe' == this.userTableName&&!this.rules['zuozhexingming']) {
			this.$set(this.rules, 'zuozhexingming', [{ required: true, message: '请输入作者姓名', trigger: 'blur' }]);
		}
		if ('zuozhe' == this.userTableName&&this.rules['mima']){
			this.rules['mima'].push({ required: true, message: '请输入密码', trigger: 'blur' })
		}else if('zuozhe' == this.userTableName&&!this.rules['mima']) {
			this.$set(this.rules, 'mima', [{ required: true, message: '请输入密码', trigger: 'blur' }]);
		}
		if ('zuozhe' == this.userTableName) {
			this.$set(this.rules, 'shoujihao', [{ required: false, validator: this.$validate.isMobile, trigger: 'blur' }]);
    ...
    methods: {
      ...
      onSubmit(formName) {
		...
		if(`zuozhe` == this.userTableName && this.sessionForm.touxiang!=null){
			this.sessionForm.touxiang = this.sessionForm.touxiang.replace(new RegExp(this.$config.baseUrl,"g"),"");
		}
      ...
      zuozhetouxiangHandleAvatarSuccess(fileUrls) {
        this.sessionForm.touxiang = fileUrls;
      },
      chongzhivip() {
