    ...
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="小说编号" prop="xiaoshuobianhao">
            <el-input v-model="ruleForm.xiaoshuobianhao" 
                placeholder="小说编号" clearable :disabled=" false  ||ro.xiaoshuobianhao"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="加入时间" prop="jiarushijian">
              <el-date-picker
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`wodeshujia/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('wodeshujia/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`wodeshujia/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
