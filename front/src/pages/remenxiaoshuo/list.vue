...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 6px","margin":"172px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","float":"right","justifyContent":"space-between","borderRadius":"12px","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"380px","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>书籍名称：</div>
        <el-input v-model="formSearch.shujimingcheng" placeholder="书籍名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('remenxiaoshuo','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 0px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#1c6a6d","width":"100px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/remenxiaoshuoAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"minHeight":"900px","padding":"20px 0","margin":"0px 0 0","borderRadius":"0px","background":"none","width":"calc(100% - 400px)","float":"left","order":"6"}'>
		...
		<div :style='{"width":"100%","padding":"0 0 0 2%","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"width":"31.3%","margin":"0 2% 30px 0","overflow":"hidden","position":"relative","background":"#fff","height":"480px"}' class="item">
			<img @click.stop="imgPreView(item.xiaoshuofengmian)" v-if="item.xiaoshuofengmian && item.xiaoshuofengmian.substr(0,4)=='http'" :src="item.xiaoshuofengmian.split(',')[0]" class="image" />
			<img @click.stop="imgPreView(baseUrl + (item.xiaoshuofengmian?item.xiaoshuofengmian.split(',')[0]:''))" v-else :src="baseUrl + (item.xiaoshuofengmian?item.xiaoshuofengmian.split(',')[0]:'')" class="image" />
			<div class="content">
		      <div class="info" :style='{"padding":"30% 0 0 0px","overflow":"hidden","alignItems":"flex-start","flexWrap":"wrap","display":"block","width":"48%","height":"100%"}'>
				<div :style='{"padding":"0 0 0 20px","borderColor":"#ddd","whiteSpace":"nowrap","color":"inherit","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">{{item.shujimingcheng}}</div>
				<div :style='{"padding":"0 0 0 20px","borderColor":"#ddd","whiteSpace":"nowrap","color":"inherit","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">{{item.xiaoshuofenlei}}</div>
		        ...
		        <div :style='{"width":"100%","padding":"0 0 0 20px","borderColor":"#ddd","borderStyle":"dashed","borderWidth":"0 0 1px","display":"none"}'>
		          ...
		          <span :style='{"color":"inherit","lineHeight":"40px","fontSize":"inherit"}'>{{item.zuozhezhanghao}}</span>
		        </div>
		        <div :style='{"width":"100%","padding":"0 0 0 20px","borderColor":"#ddd","borderStyle":"dashed","borderWidth":"0 0 0px"}'>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/xiaoshuofenlei/xiaoshuofenlei').then(res => {
			if (res.data.code == 0) {
				this.xiaoshuofenleiOptions = res.data.data;
				this.selectOptionsList.push({name:'小说分类',list:this.xiaoshuofenleiOptions,tableName: 'xiaoshuofenlei',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('xiaoshuofenlei/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'xiaoshuofenlei'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.xiaoshuofenlei != '') searchWhere.xiaoshuofenlei = this.formSearch.xiaoshuofenlei;
		if(!this.centerType){
			params['sfsh'] = '是';
		}
        if (this.curFenlei != '全部') searchWhere.xiaoshuofenlei = this.curFenlei;
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`remenxiaoshuo/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/remenxiaoshuoDetail', query: params});
      },
