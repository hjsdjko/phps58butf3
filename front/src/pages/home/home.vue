...
<div id="animate_recommendremenxiaoshuo" class="recommend animate__animated" :style='{"padding":"60px 7% 20px","margin":"0 auto","borderColor":"#ddd","background":"url(http://codegen.caihongy.cn/20231010/dab28806e6084effb4866358e938b0b4.jpg) no-repeat center top / 100% 100%","borderWidth":"0px 0","width":"100%","position":"relative","borderStyle":"solid","order":"2"}'>
    ...
    <div class="title" :style='{"width":"100%","margin":"0px auto","lineHeight":"60px","textAlign":"left","background":"url(http://codegen.caihongy.cn/20231010/84c0d484cc5144b0b970462a64801e94.png) no-repeat 520px center"}'>
		<span :style='{"padding":"0","fontSize":"50px","color":"#fff","fontWeight":"600","display":"block"}'>热门小说推荐</span>
	</div>
	...
	<div class="list list3 index-pv1">
		<div :style='{"width":"100%","padding":"10px 10px 0","margin":"60px 0","background":"#fff","height":"auto"}' class="swiper-container" id="recommendremenxiaoshuo">
			<div class="swiper-wrapper">
				<div class="swiper-slide animation-box" :style='{"border":"0","cursor":"pointer","padding":"0px 0px 20px","boxShadow":"1px 2px 4px #ccc,inset -8px 0px 6px 0px #e6e6e6","borderRadius":"0px","background":"#fff","position":"relative"}' v-for="(item,index) in remenxiaoshuoRecommend" :key="index" @click="toDetail('remenxiaoshuoDetail', item)">
					<img :name="item.id" :style='{"objectFit":"cover","width":"100%","height":"480px"}' v-if="preHttp(item.xiaoshuofengmian)" :src="item.xiaoshuofengmian.split(',')[0]" alt="" />
					<img :name="item.id" :style='{"objectFit":"cover","width":"100%","height":"480px"}' v-else :src="baseUrl + (item.xiaoshuofengmian?item.xiaoshuofengmian.split(',')[0]:'')" alt="" />
					<div class="line1" :style='{"padding":"0 10px","margin":"6px 0 0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","background":"none","width":"calc(100% - 104px)","fontSize":"14px","lineHeight":"32px","textOverflow":"ellipsis","fontWeight":"600","height":"32px"}'>{{item.shujimingcheng}}</div>
					<div class="line1" :style='{"padding":"0 10px","margin":"6px 0 0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","background":"none","width":"calc(100% - 104px)","fontSize":"14px","lineHeight":"32px","textOverflow":"ellipsis","fontWeight":"600","height":"32px"}'>{{item.xiaoshuofenlei}}</div>
					...
					<div :style='{"padding":"0 10px","position":"absolute","right":"0","top":"490px","display":"inline-block"}'>
					  ...
					  <span class="text" :style='{"color":"#999","lineHeight":"1.5","fontSize":"14px"}'>{{item.zuozhezhanghao}}</span>
					</div>
	...
	<div @click="moreBtn('remenxiaoshuo')" :style='{"border":"0px solid #fff","cursor":"pointer","margin":"12px auto","top":"60px","textAlign":"center","left":"calc(7% + 400px)","background":"none","display":"block","width":"auto","lineHeight":"36px","position":"absolute"}'>
		<span :style='{"color":"#fff","fontSize":"16px","textDecoration":"underline"}'>查看更多</span>
    ...
    methods: {
		...
		handleScroll() {
			let arr = [
				...
				{id:'animate_recommendremenxiaoshuo',css:'animate__'},
      ...
      getSystemIntroduction() {
		...
		getList() {
			let autoSortUrl = "";
			let data = {}
          autoSortUrl = "remenxiaoshuo/autoSort";
			data = {
				page: 1,
				limit: 8,
			}
			...
			this.$http.get(autoSortUrl, {params: data}).then(res => {
				if (res.data.code == 0) {
					this.remenxiaoshuoRecommend = res.data.data.list;
					...
					this.$nextTick(() => {
						new Swiper('#recommendremenxiaoshuo', options)
					})
